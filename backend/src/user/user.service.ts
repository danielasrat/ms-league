...
async getProfile(userId: string) {
  const user = await this.userModel.findById(userId).select('name phone email profileImage');
...
async getMembershipStatus(userId: string) {
  const user = await this.userModel.findById(userId).select('hasPaid daysLeft membershipExpires paymentMethod status');
  if (!user) throw new NotFoundException('User not found');
